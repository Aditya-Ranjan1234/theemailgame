...
import itertools
from src.leaderboard import (compute_leaderboard, render_leaderboard_html,
...
class EmailServer:
    ...
    def get_messages_for_agent(self, agent_id: str) -> List[Dict]:
        ...
        Only mail tagged for the agent's current game (plus any untagged/legacy
        mail) is returned, so a reconnect never replays a previous game's backlog
        and a queued agent (no current game) sees no game mail. Outside
        competition mode this returns everything (local dev/playtest)."""
        msgs = [msg for msg in self.messages if msg["to"] == agent_id]
        if not COMPETITION_MODE:
            return msgs
        current_game = self.agent_to_game.get(agent_id)
        return [m for m in msgs if m.get("game_id") in (current_game, None)]
...
def _live_dict() -> dict:
    """Point-in-time arena activity (server-global)."""
    return {
...
async def leaderboard_page():
    ...
    try:
        ...
        return HTMLResponse(render_leaderboard_html(
            compute_leaderboard(window="competition"), _live_for(board), board=board))
    except Exception as e:
...
async def leaderboard_testing_page():
    """Build-week (testing) leaderboard: games played before the competition
    starts. Clearly marked unofficial; does not count toward the competition."""
    try:
        return HTMLResponse(render_leaderboard_html(
            compute_leaderboard(window="build"), _live_for("build"), board="build"))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to build testing leaderboard: {str(e)}")
...
async def leaderboard_api():
    ...
    try:
        ...
        return {"success": True, "leaderboard": compute_leaderboard(window="competition"), "live": live}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to build leaderboard: {str(e)}")
...
async def leaderboard_testing_api():
    """Machine-readable build-week (testing) leaderboard."""
    try:
        live = _live_for("build")          # activity only during build week (active board)
        live["board"] = "build"
        return {"success": True, "leaderboard": compute_leaderboard(window="build"), "live": live}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to build testing leaderboard: {str(e)}")
